...
class DipStrategyTests(unittest.TestCase):
    ...
    @patch("yfinance.download")
    def test_get_price_data_stops_after_rate_limit_when_retries_disabled(self, mock_download, _mock_sleep):
        index = pd.date_range("2024-01-01", periods=2)
    ...
    @patch("yfinance.download")
    def test_bulk_price_data_falls_back_when_bulk_empty(self, mock_download, mock_alpaca, mock_cache):
        mock_download.return_value = pd.DataFrame()
        index = pd.date_range("2024-01-01", periods=2)
        mock_alpaca.side_effect = [
            pd.DataFrame({"Close": [100.0, 101.0]}, index=index),
        ...
        self.assertFalse(result["AAPL"].empty)
        self.assertFalse(result["MSFT"].empty)
        self.assertEqual(mock_alpaca.call_count, 2)
